...
def start_audio():
    state.stream=sd.InputStream(samplerate=SAMPLE_RATE,channels=1,
                                  dtype='float32',callback=ses_callback)
    state.stream.start()

def whisper_isle():
    if not state.audio_frames or not state.whisper:
...
def _play_animations():
    while not state.anim_queue.empty():
        tip,icerik,yol=state.anim_queue.get()
...
def _llm_eslestime(metin, kelimeler, kelime_listesi):
    ...
    prompt=(
        f"Mevcut animasyon dosyaları (bunların dışında seçim yapma): {liste_str}\n\n"
        f"Eşleştirilecek kelimeler:\n{numbered}\n\n"
        "Her kelime için yukarıdaki animasyon listesinden en uygun olanı seç.\n"
        "Kurallar:\n"
        "1. Türkçe çekim/yapım eklerini çıkar: \"annemi\"→\"anne\", \"seviyorum\"→\"sevmek\", \"eve\"→\"ev\"\n"
        "2. Kök kelime listede varsa o dosyayı seç\n"
        "3. Listede hiç karşılık yoksa null yaz\n"
        "4. SADECE JSON array döndür\n"
        f"5. Array tam {len(kelimeler)} elemanlı olmalı\n"
        "Örnek: Liste:[ben,anne,ev] Kelimeler:[benim,annemi,eve] → [\"ben\",\"anne\",\"ev\"]"
        "6. aynı animasyonu ardışık olarak kullanma\n")
    try:
        r=state.groq_client.chat.completions.create(
            model="llama-3.3-70b-versatile",
            messages=[{"role":"system","content":"Sadece geçerli JSON array döndür."},
    ...
    if eslesmeler is None or len(eslesmeler)!=len(kelimeler):
        ...
        for i,kelime in enumerate(kelimeler):
            ...
            for da in kelime_listesi:
                if _normalize(da)==temiz: eslesmeler[i]=da; break
    animler=[]
            ...
            if os.path.exists(kd):
                ...
                for f in os.listdir(kd):
                    if _normalize(f.replace(".mp4","").replace(".MP4",""))==en:
                        yol=os.path.join(kd,f); break
            if yol: animler.append(("kelime",eslesme,yol)); continue
        for h in temiz:
...
def on_kayit_baslat(data=None):
    if state.sag_state not in ("bekle",): return
    state.audio_frames=[]; state.recording=True; state.sag_state="kayit"
    sio.emit("sag_state",{"state":"kayit"})

@sio.on("kayit_bitir")
def on_kayit_bitir(data=None):
    if state.sag_state!="kayit": return
    state.recording=False; state.sag_state="isleniyor"
...
def on_sifirla(data=None):
    ...
    state.sag_state="bekle"; state.recording=False
